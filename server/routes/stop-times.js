...
router.get('/:id/transform', async (req, res) => {
  ...
  let stopTimes = {},delay = {}, timing = [];timing1 = [];timing2 = new Map();timing3 = [];timing4 = [];
  let timingMap = []; var tempTimingMap = new Map();
  await axios.get('http://localhost:8080/api/vehicle/', config)
  .then(vehicleRes => {
  ...
  await axios.get('http://localhost:8080/api/stop-times/'+stopId, config)
    .then(async stoptimeRes => {
      for(let i=0;i< (stoptimeRes.data.length);i++){
        ...
        stopTimes[stoptimeRes.data[i].tripId] = 
         ...
         await axios.get('http://localhost:8080/api/trips/'+stoptimeRes.data[i].tripId, config)
           .then(tripRes => {
        ...
        timing1.push(tripRes.data[0].routeId+
